...
public class FreeChatSessionService extends SessionService {
	...
	protected void saveTranscript(String localSessionId, SessionEvent event) {
		...
		RealtimeFlowLog.info("flow.event.transcript localSessionId={} eventType={} rawType={} speaker={} transcript={}",
				localSessionId, event.type(), event.rawType(), event.speaker(),
				RealtimeFlowLog.textSummary(event.transcript()));
	}
