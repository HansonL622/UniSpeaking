...
UniSpeaking 是一个 AI 英语口语陪练产品。目前仓库中的前端界面仍在按模块持续设计、修改与定稿。本文档记录截至 **2026-07-22** 的可联调状态。

## 当前进度

当前版本已覆盖自由对话、场景广场、自定义场景“学 → 读 → 说”流程、学习资产和个人中心主要页面。除特别说明外，业务数据仍为前端 Mock，不能视为接口已经接入。
...
| 学习资产 | 场景资产主页与模拟对话详情已完成；IELTS / 英文面试仅预留入口 |
...
主要页面已有 `/conversation`、`/scenes`、`/training`、`/assets`、`/profile`、`/membership` 和 `/settings` 等可刷新路径，但仍未建立正式路由层。刷新后业务 state 会恢复为演示默认值，当前没有接入 Local Storage、服务端 Session 或数据库。
...
- 普通场景学习资产与 IELTS、英文面试资产分开。后两者当前只预留入口，权限、内容结构和独立学习路径尚未定稿。
...
- 没有无障碍完整审计和多语言架构。
- IELTS、英文面试的独立学习路径与学习资产尚未设计，目前仅预留入口。
