  ...
  if (!inCall) return (
    <main className="conversation standby">
      ...
      <section className="standby__center">
        ...
        <h1>想聊什么都可以</h1>
        <p>像打电话一样自然开口，后端会通过 WebRTC 连接实时模型。</p>
  ...
  return (
    <main className={cx("conversation call", subtitles && "call--subtitles")}>
      ...
      <section className="call__stage">
        <div className={cx("call-presence", subtitles && "call-presence--compact")}>
          ...
          <div className={cx("listening-state", subtitles && "listening-state--compact")}>
            ...
            <time className="call-presence__time">{callState === "connecting" ? "连接中" : callState === "ended" ? "已结束" : paused ? "已暂停" : "实时中"}</time>
      ...
      <div className="call-controls">
        <button className={cx("round-control", paused && "is-on")} aria-label={paused ? "恢复会话" : "暂停会话"} disabled={callState === "ended"} onClick={togglePaused}>{paused ? <MicrophoneSlash /> : <Microphone />}</button>
        ...
        <button className="round-control round-control--end" aria-label="结束当前会话" disabled={callState === "ended"} onClick={stopConversation}><PhoneDisconnect weight="fill" /></button>
      </div>
    </main>
  );
}
...
function routeFromLocation() {
  ...
  if (preview) {
    ...
    if (preview === "result") return { flow: "app", page: "scenes", authMode: "signup", training: null, result: { completed: true } };
  ...
  if (path === "/result") return { flow: "app", page: "scenes", authMode: "signup", training: null, result: { completed: true } };
...
export function App() {
  ...
  const showResult = (completed) => navigate("/result", { flow: "app", page: "scenes", authMode, training: null, result: { completed } });
  ...
  let content;
  if (training) content = <Training sceneTitle={sceneTitle} teacher={teacher} initialStep={training.initialStep} onExit={() => setMainPage("scenes")} onComplete={showResult} />;
  else if (result) content = <Result completed={result.completed} onBack={() => setMainPage("scenes")} onAssets={() => setMainPage("assets")} />;
